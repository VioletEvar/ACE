...
LOSS_THRESHOLD = 10.0  # loss threshhold
...
def fit_frame(
    ...
    params, hand_verts, total_loss = optimize_mano_shape(
        target_v3d,
        mano_layers,
        optim_specs,
        iteration,
    ...
    return params, hand_verts, total_loss
    ...
    for iteration, v3d_ra in pbar:
        ...
        if is_valid or prev_v3d_ra is not None:
            ...
            if loss <= LOSS_THRESHOLD:
                prev_v3d_ra = copy.deepcopy(v3d_ra)  # save v3d data in current frame for possible reuse 
            else:
                out, hand_verts, loss = fit_frame(
                    seq_name,
                    prev_v3d_ra,
